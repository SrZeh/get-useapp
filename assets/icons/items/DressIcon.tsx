import React from 'react';
import { Svg, Path, G } from 'react-native-svg';

type DressIconProps = React.ComponentProps<typeof Svg> & {
  size?: number;
};

export function DressIcon({ size = 96, ...rest }: DressIconProps) {
  const width = rest.width ?? size;
  const height = rest.height ?? size;

  return (
    <Svg viewBox="0 0 512 512" width={width} height={height} fill="none" {...rest}>
      <G>
        <Path
          fill="#26B99A"
          d="M365.124,67.013c15.528-24.806,2.066-57.415-26.527-63.629C327.607,1,318.77,1,318.77,1
          c-52.966,0-61.793,26.483-61.793,26.483S248.149,1,195.184,1c0,0-8.836,0-19.827,2.383c-28.593,6.215-42.055,38.824-26.527,63.629
          c1.474,2.357,2.993,4.864,4.52,7.512c20.86,36.043,21.142,80.552,0.865,116.921c-2.216,3.972-4.537,8.174-6.947,12.588h219.418
          c-2.41-4.414-4.732-8.616-6.947-12.588c-20.277-36.37-19.994-80.878,0.865-116.921C362.131,71.877,363.65,69.37,365.124,67.013"
        />
        <G>
          <Path
            fill="#1C8E73"
            d="M248.227,62.79c0,4.873,3.955,8.828,8.828,8.828s8.828-3.955,8.828-8.828s-3.955-8.828-8.828-8.828
            S248.227,57.918,248.227,62.79"
          />
          <Path
            fill="#1C8E73"
            d="M248.227,98.101c0,4.873,3.955,8.828,8.828,8.828s8.828-3.955,8.828-8.828
            c0-4.873-3.955-8.828-8.828-8.828S248.227,93.228,248.227,98.101"
          />
          <Path
            fill="#1C8E73"
            d="M248.227,133.411c0,4.873,3.955,8.828,8.828,8.828s8.828-3.955,8.828-8.828
            c0-4.873-3.955-8.828-8.828-8.828S248.227,128.538,248.227,133.411"
          />
          <Path
            fill="#1C8E73"
            d="M248.227,168.721c0,4.873,3.955,8.828,8.828,8.828s8.828-3.955,8.828-8.828
            c0-4.873-3.955-8.828-8.828-8.828S248.227,163.849,248.227,168.721"
          />
        </G>
        <Path
          fill="#26B99A"
          d="M147.255,204.105c-36.59,66.957-93.29,181.327-93.29,247.102c0,0-8.324,58.465,185.432,61.793
          c5.694,0.097,11.555-44.138,17.602-44.138c6.082,0,11.988,44.235,17.708,44.138c193.66-3.346,185.326-61.793,185.326-61.793
          c0-65.748-56.647-180.047-93.237-247.014l-0.088-0.159H147.3L147.255,204.105z"
        />
        <G>
          <Path
            fill="#1C8E73"
            d="M458.005,461.07c2.586-5.906,2.03-9.869,2.03-9.869c0-65.792-56.735-180.224-93.325-247.172
            H257.054C257.054,328.374,342.549,432.522,458.005,461.07"
          />
          <Path
            fill="#1C8E73"
            d="M257.055,204.032H147.293c-36.582,66.948-93.325,181.372-93.325,247.172
            c0,0-0.547,3.972,2.039,9.887C171.515,432.578,257.055,328.413,257.055,204.032"
          />
        </G>
      </G>
    </Svg>
  );
}
